...
pub mod ffi {
    ...
    unsafe extern "C++" {
        ...
        fn init_null_config() -> UniquePtr<NativeNode_init_pose>;
        ...
        fn get_init_pose(node: &mut UniquePtr<NativeNode_init_pose>)
            -> GeometryMsgsPoseWithCovarianceStamped;
        fn is_new(node: &mut UniquePtr<NativeNode_init_pose>) -> bool;
    }
}
...
impl Debug for ffi::NativeNode_init_pose {
    fn fmt(&self, f: &mut Formatter<'_>) -> Result {
        f.debug_struct(type_name::<ffi::NativeNode_init_pose>()).finish()
    }
}
